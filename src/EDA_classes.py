...
class DistributionAnalyzer(EDA):
    ...
    def plot_numerical_density(self, cols: list[str] = None) -> None:
        """
        ...
        if cols is None:
            ...
            numerical_cols = [col for col in cols if col in self.df.columns]
            numerical_cols = self.df[numerical_cols].select_dtypes(include=[np.number]).columns.to_list()

        num_plots = len(numerical_cols)
        fig_rows = int(np.ceil(num_plots / 3))

        self.df[numerical_cols].plot(
            kind='density',
        ...
        plt.suptitle("Density plot of Numerical features", fontsize=20, y=1.02)
        plt.tight_layout()
        plt.show()

    def plot_numerical_boxplot(self, cols: list[str] = None) -> None:
        """
        ...
        plt.suptitle("Box plot of Numerical features", fontsize=20, y=1.02)
        plt.tight_layout()
        plt.show()
    ...
    def plot_categorical_distribution(self, cols: list[str] = None) -> None:
        """
        ...
        if cols is None:
            ...
            categorical_cols = [col for col in cols if col in self.df.columns]
            categorical_cols = self.df[categorical_cols].select_dtypes(include=['object', 'category']).columns.to_list()
        ...
        for i, col in enumerate(categorical_cols):
            ...
            sns.countplot(x=col, data=self.df, palette='viridis', order = order)
